...
class Wallet {
    ...
    async _generateOrbitKeys(seed) {
        return new Promise(function(resolve, reject) {
            try {
                ...
                //the path:m/44'/60'/0' or let node=let node = hdkey.derive("m/44'/60'/0'). then acces the xpub via node.xpubkey
    ...
    async getOrbitDB(ipfs, repo, options) {
      const self = this;
      return new Promise(async function(resolve, reject) {
        try {
          options = options || {}
          ...
          const orbitdb = new OrbitDB(ipfs, repo, options);
          resolve(orbitdb);
        } catch(e) {
          reject(e);
        }
